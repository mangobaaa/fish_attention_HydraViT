...
def build_model_with_cfg(
    ...
    if not isinstance(variant, str):
        if len(variant) > 1:
            pretrained_cfg2 = resolve_pretrained_cfg(
                variant[1],
                pretrained_cfg=pretrained_cfg,
                pretrained_cfg_overlay=
                pretrained_cfg_overlay
            )
            pretrained_cfg2 = pretrained_cfg2.to_dict()
            _update_default_model_kwargs(pretrained_cfg2, kwargs, kwargs_filter)
        variant = variant[0]
    ...
    if pretrained:
        ...
        if isinstance(tmp, list):
            if len(tmp) > 1:
                model.pretrained_cfg = pretrained_cfg2
                model.default_cfg = model.pretrained_cfg  # alias for backwards compat
                load_pretrained(
                    model,
                    pretrained_cfg=pretrained_cfg2,
                    num_classes=num_classes_pretrained,
                    in_chans=kwargs.get('in_chans', 3),
                    filter_fn=pretrained_filter_fn,
                    strict=pretrained_strict,
